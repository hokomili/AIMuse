...
| Development package structure | Fresh local arm64 ad-hoc package passed architecture, ASAR/native resources, bundle identity, signature structure and exact hardened-fuse verification | Repeat on final immutable formal subjects and every shipping architecture; this is not Developer ID, notarization or distribution evidence. |
...
The 2026-08-10 arm64 Level 2 result remains a historical certificate for its exact subject only. The current local checkpoint adds clean dependency/package construction, exact installed bridge execution, device-free native protocol probes and an installed stdio-client headless lifecycle. It used only ad-hoc signing, never touched Keychain or an audio/MIDI device, and makes no Developer ID, Gatekeeper, notarization, stapling, GUI, device, distribution or Level 3 claim.

A future macOS acceptance must preserve the provider/storage removal and extend the now-observed installed headless/static-client path to interactive GUI/visual activation, named client versions, exact-envelope engine/device work and the retained native DAW workflows. Level 3 remains open.
