...
- [ ] Before automation, remove only the ignored generated `.vite` staging directory, then exclusively publish schema-2 declared inputs that bind the manifest-only source with Vite/Vitest environment-file loading, Vite public copying and PostCSS discovery disabled; exact direct tool bytes; a problem-free logical dependency graph; every contract-declared loaded runtime/support-tree content inventory; a clean pinned miniaudio tree copied into the formal root; private home/temp/cache and fresh native build/dist paths; release contract, witness, and verifier controls.
...
- [ ] Complete a distinct-task Level 2 Astra/high MCP + native Computer Use report and schema-2 case manifest against that exact package, with case-exclusive review evidence and no Blocker/P0/P1, then have caller-pinned `release-level-certifier.mjs` re-derive automation, bind every case/report artifact, prove cleanup and post-stop package identity, and derive the full Level 2 result.
