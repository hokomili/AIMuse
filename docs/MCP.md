...
Direct and branch `project_apply` plus `history_manage` undo/redo pass through four shared admission lanes. Queued work is served round-robin by authenticated actor, with at most four queued mutations per actor and a global queued cost of 2,048; a project transaction costs its operation count and one history action costs one. Overflow returns `status: "busy"` with `actor_queue_full` or `global_queue_full`, a bounded retry delay, queue depths and explicit next-step guidance; it does not mutate the project. Because undo/redo is not idempotent, its busy/cancelled guidance requires re-observation before the caller decides whether to issue another history action rather than instructing a blind retry.

Closing an MCP session cancels that actor’s queued work, while **Stop agents** cancels matching queued project work. Queued history cancellation occurs before the actor history method is invoked, so it cannot partially apply an undo or redo. Already-running reducer and atomic history commits are not preempted, and the project service continues to serialize commits to the same project. Dedicated headless acceptance exercises this contract both in memory and through bearer-authenticated loopback without starting audio; packaged coverage remains separate. Checkpoint/variant, media, generation, plug-in and other domain-tool mutations have not yet been moved behind this admission layer; their active-work and partial-commit policy remains open.
