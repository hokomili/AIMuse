...
/** Single source for runtime validation and public operation-payload discovery. */
export function projectOperationSchema(kind: ProjectOperation['kind']): z.ZodType {
  const raw = { kind };
  switch (raw.kind) {
    case 'project.rename': return z.object({ kind: z.literal(raw.kind), name: z.string().min(1).max(200) }).strict();
    case 'project.settings.update': return z.object({ kind: z.literal(raw.kind), changes: z.object({ sampleRate: z.union([z.literal(44_100), z.literal(48_000), z.literal(96_000)]).optional(), channelLayout: z.enum(['mono', 'stereo']).optional(), ppq: z.literal(960).optional(), recordBitDepth: z.union([z.literal(24), z.literal(32)]).optional(), countInBars: z.number().int().min(0).max(8).optional(), metronomeEnabled: z.boolean().optional(), defaultCrossfadeTicks: TickSchema.optional(), masterLufsTarget: FiniteSchema.min(-36).max(-5).optional() }).strict() }).strict();
    case 'tempo.upsert': return z.object({ kind: z.literal(raw.kind), event: TempoSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'tempo.delete': case 'meter.delete': return z.object({ kind: z.literal(raw.kind), eventId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'meter.upsert': return z.object({ kind: z.literal(raw.kind), event: MeterSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'marker.add': return z.object({ kind: z.literal(raw.kind), marker: MarkerSchema, index: z.number().int().nonnegative().optional() }).strict();
    case 'marker.update': return z.object({ kind: z.literal(raw.kind), markerId: IdSchema, changes: z.object({ tick: TickSchema.optional(), endTick: TickSchema.optional(), name: z.string().min(1).max(200).optional(), color: z.string().max(40).optional(), kind: z.enum(['marker', 'region', 'cue']).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'marker.delete': return z.object({ kind: z.literal(raw.kind), markerId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'section.add': return z.object({ kind: z.literal(raw.kind), section: SectionSchema, index: z.number().int().nonnegative().optional() }).strict();
    case 'section.update': return z.object({ kind: z.literal(raw.kind), sectionId: IdSchema, changes: z.object({ name: z.string().min(1).max(200).optional(), startTick: TickSchema.optional(), endTick: TickSchema.optional(), color: z.string().max(40).optional(), energy: FiniteSchema.min(0).max(1).optional(), prompt: z.string().max(10_000).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'section.delete': return z.object({ kind: z.literal(raw.kind), sectionId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'lyrics.set': return z.object({ kind: z.literal(raw.kind), lyrics: z.string().max(200_000) }).strict();
    case 'track.add': return z.object({ kind: z.literal(raw.kind), track: TrackSchema, index: z.number().int().nonnegative().optional(), parentId: IdSchema.optional() }).strict();
    case 'track.update': return z.object({ kind: z.literal(raw.kind), trackId: IdSchema, changes: z.object({ name: z.string().min(1).max(200).optional(), color: z.string().max(40).optional(), gainDb: FiniteSchema.min(-120).max(24).optional(), pan: FiniteSchema.min(-1).max(1).optional(), mute: z.boolean().optional(), solo: z.boolean().optional(), armed: z.boolean().optional(), frozen: z.boolean().optional(), collapsed: z.boolean().optional(), routing: RoutingSchema.optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'track.move': return z.object({ kind: z.literal(raw.kind), trackId: IdSchema, index: z.number().int().nonnegative(), parentId: IdSchema.optional(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'track.delete': return z.object({ kind: z.literal(raw.kind), trackId: IdSchema, cascade: z.boolean(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'clip.add': return z.object({ kind: z.literal(raw.kind), clip: ClipSchema, index: z.number().int().nonnegative().optional() }).strict();
    case 'clip.update': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, changes: z.object({ name: z.string().min(1).max(200).optional(), color: z.string().max(40).optional(), startTick: TickSchema.optional(), durationTicks: z.number().int().positive().optional(), muted: z.boolean().optional(), gainDb: FiniteSchema.min(-120).max(24).optional(), fadeIn: FadeSchema.optional(), fadeOut: FadeSchema.optional(), loopEnabled: z.boolean().optional(), loopLengthTicks: z.number().int().positive().optional(), sourceStartSample: z.number().int().nonnegative().optional(), sourceDurationSamples: z.number().int().positive().optional(), transposeSemitones: FiniteSchema.min(-48).max(48).optional(), stretchMode: z.enum(['repitch', 'stretch']).optional(), reverse: z.boolean().optional(), warpMarkers: z.array(WarpSchema).max(100_000).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'clip.move': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, trackId: IdSchema, startTick: TickSchema, index: z.number().int().nonnegative().optional(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'clip.trim': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, startTick: TickSchema, durationTicks: z.number().int().positive(), sourceStartSample: z.number().int().nonnegative().optional(), sourceDurationSamples: z.number().int().positive().optional(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'clip.split': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, tick: TickSchema, rightClip: ClipSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'clip.delete': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'take-lane.add': return z.object({ kind: z.literal(raw.kind), lane: TakeLaneSchema }).strict();
    case 'take-lane.update': return z.object({ kind: z.literal(raw.kind), laneId: IdSchema, changes: z.object({ name: z.string().min(1).max(200).optional(), active: z.boolean().optional(), clipIds: z.array(IdSchema).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'take-lane.delete': return z.object({ kind: z.literal(raw.kind), laneId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'comp-segment.upsert': return z.object({ kind: z.literal(raw.kind), segment: CompSegmentSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'comp-segment.delete': return z.object({ kind: z.literal(raw.kind), segmentId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.note.add': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, note: NoteSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.note.update': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, noteId: IdSchema, changes: z.object({ startTick: TickSchema.optional(), durationTicks: z.number().int().positive().optional(), pitch: z.number().int().min(0).max(127).optional(), velocity: FiniteSchema.min(0).max(1).optional(), releaseVelocity: FiniteSchema.min(0).max(1).optional(), channel: z.number().int().min(0).max(15).optional(), probability: FiniteSchema.min(0).max(1).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.note.delete': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, noteId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.control.add': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, event: ControlSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.control.delete': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, eventId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.pitch-bend.add': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, event: PitchBendSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.pitch-bend.update': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, eventId: IdSchema, changes: z.object({ tick: TickSchema.optional(), value: FiniteSchema.min(-1).max(1).optional(), channel: z.number().int().min(0).max(15).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.pitch-bend.delete': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, eventId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'midi.semantic': return z.object({ kind: z.literal(raw.kind), clipId: IdSchema, noteIds: z.array(IdSchema).max(100_000).optional(), action: z.enum(['quantize', 'humanize', 'transpose', 'legato', 'duplicate', 'arpeggiate']), gridTicks: z.number().int().positive().optional(), strength: FiniteSchema.min(0).max(1).optional(), seed: z.number().int().optional(), timingTicks: z.number().int().nonnegative().optional(), velocityAmount: FiniteSchema.min(0).max(1).optional(), semitones: z.number().int().min(-127).max(127).optional(), gapTicks: z.number().int().optional(), offsetTicks: z.number().int().optional(), stepTicks: z.number().int().positive().optional(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'automation.lane.add': return z.object({ kind: z.literal(raw.kind), lane: LaneSchema }).strict();
    case 'automation.lane.update': return z.object({ kind: z.literal(raw.kind), laneId: IdSchema, changes: z.object({ armed: z.boolean().optional(), visible: z.boolean().optional(), target: TargetSchema.optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'automation.lane.delete': return z.object({ kind: z.literal(raw.kind), laneId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    ...
    case 'send.upsert': return z.object({ kind: z.literal(raw.kind), send: SendSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'send.delete': return z.object({ kind: z.literal(raw.kind), sendId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'sidechain.upsert': return z.object({ kind: z.literal(raw.kind), route: SidechainSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'sidechain.delete': return z.object({ kind: z.literal(raw.kind), routeId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'asset.add': return z.object({ kind: z.literal(raw.kind), asset: AssetSchema }).strict();
    case 'asset.delete': return z.object({ kind: z.literal(raw.kind), assetId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'provenance.register': return z.object({ kind: z.literal(raw.kind), provenance: ProvenanceSchema }).strict();
    case 'provenance.update': return z.object({ kind: z.literal(raw.kind), provenanceId: IdSchema, changes: z.object({ transformations: z.array(z.string().max(500)).max(200).optional(), modelVersion: z.string().max(100).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'sfx-deliverable.add': return z.object({ kind: z.literal(raw.kind), deliverable: DeliverableSchema }).strict();
    case 'sfx-deliverable.update': return z.object({ kind: z.literal(raw.kind), deliverableId: IdSchema, changes: z.object({ name: z.string().min(1).max(500).optional(), startTick: TickSchema.optional(), endTick: TickSchema.optional(), variantCount: z.number().int().min(1).max(1000).optional(), tags: z.array(z.string().max(100)).max(100).optional(), seamlessLoop: z.boolean().optional(), loopStartSample: z.number().int().nonnegative().optional(), loopEndSample: z.number().int().nonnegative().optional(), tailMilliseconds: z.number().int().min(0).max(60_000).optional(), variation: VariationSchema.optional(), targetLufs: FiniteSchema.min(-36).max(-5).optional(), namingTemplate: z.string().min(1).max(500).optional(), exportFormat: z.enum(['wav', 'flac', 'mp3']).optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'sfx-deliverable.delete': return z.object({ kind: z.literal(raw.kind), deliverableId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'checkpoint.register': return z.object({ kind: z.literal(raw.kind), checkpoint: CheckpointSchema }).strict();
    case 'checkpoint.delete': return z.object({ kind: z.literal(raw.kind), checkpointId: IdSchema, expectedRevision: z.number().int().nonnegative().optional() }).strict();
    case 'variant.register': return z.object({ kind: z.literal(raw.kind), variant: VariantSchema }).strict();
    case 'variant.update': return z.object({ kind: z.literal(raw.kind), variantId: IdSchema, changes: z.object({ name: z.string().min(1).max(500).optional(), status: z.enum(['active', 'merged', 'discarded']).optional(), projectRevision: z.number().int().nonnegative().optional(), snapshotAssetId: IdSchema.optional() }).strict(), expectedRevision: z.number().int().nonnegative().optional() }).strict();
  }
}

function validateOperation(value: unknown): ProjectOperation {
  const raw = RawOperationSchema.parse(value);
  ExpectedSchema.parse(value);
  return projectOperationSchema(raw.kind).parse(value) as ProjectOperation;
}
