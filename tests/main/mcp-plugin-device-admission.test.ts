...
describe('headless MCP plug-in device admission', () => {
  ...
  beforeEach(async () => {
    ...
    host = new McpHost({ appVersion: 'test', profileId: 'E'.repeat(64), portSettingsPath: join(root, 'mcp-port.json'), cacheRoot: join(root, 'managed'), projects, audio, authority, media, plugins, generation, exports });
  ...
  it('fairly admits authenticated plug-in parameter, preset, bypass, and remove transactions with queued-only cancellation', async () => {
    ...
    const token = 'plugin-device-listener-token-0123456789';
