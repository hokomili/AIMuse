import { SoundFontControls } from './SoundFontControls';
...
const instruments: Array<{ kind: BuiltinDeviceKind; name: string; detail: string; glyph: string }> = [
  { kind: 'soundfont', name: 'SoundFont', detail: 'Piano · strings · bass · drums · 287 presets', glyph: '♫' },
...
function AssetRow({ projectId, asset, playing, onPreview }: { projectId: string; asset: MediaAsset; playing: boolean; onPreview(id: string, url: string): void }) {
  ...
  return asset.kind !== 'soundfont' && asset.mimeType.startsWith('audio/') ? <button className={`asset-row ${playing ? 'active' : ''}`} onClick={() => onPreview(`asset:${asset.id}`, window.aimuse.mediaUrl(projectId, asset.id))} aria-label={`${playing ? 'Stop' : 'Preview'} ${asset.name}`} aria-pressed={playing} title={playing ? 'Stop preview' : 'Preview audio'}>{content}</button> : <div className="asset-row" title={asset.kind === 'soundfont' ? 'Select this library in a SoundFont instrument' : 'MIDI is placed in the arrangement when imported'}>{content}</div>;
}
...
function PluginBrowser({ plugins, onScanPlugins, query }: Pick<LeftBrowserProps, 'plugins' | 'onScanPlugins'> & { query: string }) {
  ...
  return <div className="browser-section"><div className="section-heading"><span>VST3 & CLAP · Planned</span><button disabled title="Third-party plug-in hosting is planned for a later release" onClick={onScanPlugins}><RefreshCw size={12} /> Scan</button></div>{filtered.length ? filtered.map((plugin) => <div className={`plugin-row ${plugin.quarantined ? 'quarantined' : ''}`} key={plugin.id}><span><PlugZap size={15} /></span><div><strong>{plugin.name}</strong><small>{plugin.vendor} · {plugin.format.toUpperCase()}</small></div>{plugin.quarantined && <ShieldAlert size={14} />}</div>) : plugins.length ? <span className="muted-copy">No plug-ins match this search.</span> : <BrowserEmpty icon={<PlugZap size={25} />} title="Plug-in hosting is planned" action="Available in a future release" detail="Use the built-in instruments, SoundFonts and supported effects. Third-party VST3 and CLAP processing will arrive in a later release." />}</div>;
}
