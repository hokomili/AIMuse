...
import type { GenerationRequest, ProviderCapabilities } from '../common/generation';
...
export function App() {
  ...
  useEffect(() => {
    void window.aimuse.getProviderCapabilities().then(setCapabilities).catch(() => undefined);
  ...
  const configureSelectedAgent = async () => {
    setAgentSetupPending(true);
    try {
      const result = await perform(() => window.aimuse.configureAgentClient(agentClient));
      if (!result) return;
      setAgentSetup(result);
      if (result.status === 'configured') workspace.notify(`${result.clientName} is configured`, 'success');
      else if (result.status === 'cancelled') workspace.notify(result.message);
    } finally {
  ...
  const generationStart = (request: GenerationRequest) => void perform(() => window.aimuse.generationStart(request), () => { setRightVisible(true); setRightTab('jobs'); workspace.notify('Generation request started'); });
  const saveProviderCredential = async (provider: GenerationRequest['provider'], value: string) => {
    const saved = await perform(() => window.aimuse.setProviderCredential(provider, value));
  ...
  return <div className={`app-shell ${leftVisible ? '' : 'left-hidden'} ${rightVisible ? '' : 'right-hidden'}`}>
    ...
    {project ? <>
      ...
      <main className="studio-layout">
        ...
        {rightVisible && <RightSidebar project={project} selection={selection} jobs={snapshot.jobs} mcp={snapshot.mcp} tab={rightTab} mcpToken={mcpToken} onTab={setRightTab} onApply={apply} onResolveJob={(jobId, decision) => void perform(() => window.aimuse.resolveJob(jobId, decision))} onCancelJob={(jobId) => void perform(() => window.aimuse.cancelJob(jobId))} onCheckpoint={createCheckpoint} onRestoreCheckpoint={(checkpointId) => void perform(() => window.aimuse.restoreCheckpoint(project.id, checkpointId), (result) => { if (result.status !== 'committed') workspace.notify(result.message ?? `Checkpoint was not restored (${result.status})`, 'warning'); })} onStopAgents={() => void perform(() => window.aimuse.stopAgents(project.id), (count) => workspace.notify(`Stopped ${count} agent job${count === 1 ? '' : 's'}`))} onRevealCredentials={() => void perform(() => window.aimuse.getMcpCredentials(), (credentials) => setMcpToken(credentials.token))} onConnectAgent={() => { setAgentSetup(undefined); setAgentDialog(true); }} onBrowseDevices={browseDevices} notify={workspace.notify} />}
      </main>
    </> : <Welcome onNew={(kind) => setNewDialog(kind)} onOpen={openProjects} onAgents={() => setAgentDialog(true)} />}

    {newDialog && <div className="modal-backdrop" onPointerDown={() => setNewDialog(undefined)}><form className="new-project-dialog" onSubmit={(event) => { event.preventDefault(); void workspace.createProject(newDialog, newName.trim() || undefined); setNewDialog(undefined); setNewName(''); }} onPointerDown={(event) => event.stopPropagation()}><button type="button" className="modal-close" onClick={() => setNewDialog(undefined)}><X size={16} /></button><span className={`new-project-icon ${newDialog}`}>{newDialog === 'song' ? <Music2Icon kind="song" /> : <Zap size={24} />}</span><h2>New {newDialog === 'song' ? 'Song' : 'SFX Project'}</h2><p>{newDialog === 'song' ? 'Record, compose, arrange, mix, generate, and master in one project.' : 'Layer, vary, process, tag, and batch-export production-ready sounds.'}</p><label>Project name<input autoFocus value={newName} onChange={(event) => setNewName(event.target.value)} placeholder={newDialog === 'song' ? 'Untitled Song' : 'Untitled SFX'} /></label><div className="template-summary"><span><strong>48 kHz</strong> sample rate</span><span><strong>32-bit float</strong> engine</span><span><strong>960 PPQ</strong> timeline</span></div><div className="modal-actions"><button type="button" onClick={() => setNewDialog(undefined)}>Cancel</button><button type="submit" className="primary">Create project</button></div></form></div>}
    {agentDialog && <div className="modal-backdrop" onPointerDown={() => { setAgentDialog(false); setAgentSetup(undefined); }}><section className="agent-connect-dialog" onPointerDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => { setAgentDialog(false); setAgentSetup(undefined); }}><X size={16} /></button><span className="new-project-icon song"><Bot size={23} /></span><h2>Connect an external agent</h2><p>Choose a client for guided setup. Automatic setup asks before writing, backs up an existing config, and replaces only AIMuse’s entry.</p><label>MCP client<select autoFocus value={agentClient} onChange={(event) => { setAgentClient(event.target.value as AgentClientId); setAgentSetup(undefined); }}>{AGENT_CLIENTS.map((client) => <option value={client.id} key={client.id}>{client.name}</option>)}</select></label><label>MCP URL<code>{snapshot.mcp.url ?? 'Starting…'}</code></label><label>Bearer token<code>{mcpToken ?? `••••••••••••${snapshot.mcp.tokenHint ?? ''}`}</code></label>{agentSetup && <div className={`agent-setup-result ${agentSetup.status}`}><strong>{agentSetup.clientName} · {agentSetup.status}</strong><p>{agentSetup.message}</p>{agentSetup.restartRequired && <small>{agentSetup.restartInstruction}</small>}{agentSetup.setupSnippet && <><pre>{agentSetup.setupSnippet}</pre><button onClick={() => void navigator.clipboard.writeText(agentSetup.setupSnippet!).then(() => workspace.notify('Connection settings copied')).catch((cause) => workspace.notify(`Copy failed: ${cause instanceof Error ? cause.message : String(cause)}`, 'warning'))}>Copy settings</button></>}<code className="agent-docs-url">{agentSetup.documentationUrl}</code></div>}<div className="modal-actions"><button onClick={() => void perform(() => window.aimuse.getMcpCredentials(), (credentials) => setMcpToken(credentials.token))}>{mcpToken ? 'Refresh token' : 'Reveal token'}</button><button className="primary" disabled={!snapshot.mcp.url || agentSetupPending} onClick={() => void configureSelectedAgent()}>{agentSetupPending ? 'Configuring…' : agentClient === 'generic' ? 'Show settings' : `Configure ${selectedAgentDescriptor.name}`}</button></div></section></div>}
