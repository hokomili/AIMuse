...
export class MediaManager {
  ...
  async createAudioClip(projectId: Id, assetId: Id, trackId: Id, startTick: number, actor: Actor = HUMAN_ACTOR): Promise<AudioClip> {
    ...
    const clip: AudioClip = { ...entityBase('clip', actor), kind: 'audio', trackId, assetId, name: asset.name.replace(/\.[^.]+$/, ''), color: '#06b6d4', startTick: Math.max(0, Math.round(startTick)), durationTicks, muted: false, gainDb: 0, fadeIn: { durationTicks: 0, curve: 'equal-power' }, fadeOut: { durationTicks: 0, curve: 'equal-power' }, loopEnabled: false, sourceStartSample: 0, sourceDurationSamples: durationSamples, transposeSemitones: 0, stretchMode: 'stretch', reverse: false, warpMarkers: [] };
