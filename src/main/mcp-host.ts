...
export class McpHost {
  ...
  private buildServer(session: McpSession): McpServer {
    ...
    server.registerTool('project_apply', { title: 'Apply AIMuse transaction', description: 'Fairly schedules up to 512 idempotent content operations. Returns status plus revision/transaction/checkpoint, bounded conflict, or explicit retryable backpressure; AIMuse supplies trusted actor, time, and attribution metadata.', inputSchema: ProjectApplySchema, ...TOOL_OUTPUT, annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false } }, async ({ projectId, variantId, clientOperationId, label, operations, commitMode }) => {
