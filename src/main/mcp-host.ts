...
function generationJobNext(job: AsyncJob): NextStep | undefined {
  if (job.kind !== 'generation' || job.status !== 'cancelled') return undefined;
  const result = typeof job.result === 'object' && job.result ? job.result as Partial<GenerationJobResult> : undefined; const partial = result?.partial; if (!partial?.provider) return undefined;
  const provider = partial.provider.lateFulfilledRequests ? `${partial.provider.lateFulfilledRequests} submitted provider request${partial.provider.lateFulfilledRequests === 1 ? '' : 's'} fulfilled after cancellation` : partial.provider.fulfilledRequests ? `${partial.provider.fulfilledRequests} provider request${partial.provider.fulfilledRequests === 1 ? '' : 's'} fulfilled before cancellation` : partial.provider.submittedRequests ? 'a submitted provider request may still finish or charge' : 'no provider request was submitted';
  const cache = partial.candidateCache === 'retained' ? `${partial.retainedCandidateCount} immutable candidate${partial.retainedCandidateCount === 1 ? '' : 's'} remain owner-reviewable in managed cache` : partial.candidateCache === 'may-be-partial' ? 'a candidate-cache write was running and any retained effect may remain unconfirmed' : 'no candidate-cache write started';
  return { tool: 'job_manage', arguments: { action: 'inspect', jobId: job.id }, guidance: `Cancellation is terminal: ${provider}; ${cache}; generation itself made no project transaction; and no provider request was retried automatically. Inspect this owner job again before deciding whether to leave, reject, or explicitly accept a retained candidate. A later explicit acceptance is a separate project mutation.` };
}
...
function helpTopic(topic = 'getting-started'): Record<string, unknown> {
  const topics: Record<string, Record<string, unknown>> = {
    ...
    'jobs-and-approvals': { guidance: 'Every job belongs to one authenticated actor. Use job_manage list/inspect/wait/cancel. waiting-for-user means a human must decide in AIMuse; MCP cannot approve. Submit one approval-capable request at a time: a concurrent request fails with approval_pending, creates no job, and reveals no incumbent owner, job, or request details. Foreign and missing IDs both return job_not_found. Approved media import cancellation is terminal. Waiting cancellation has no file/project effects; running cancellation does not preempt the current sequential file, so inspect per-file source/cache/transaction/source-registration/warning effects and re-observe before repetition. Earlier commits and cache output remain, with no rollback, cleanup, or retry. Approved project-unpack cancellation is terminal. Waiting cancellation starts no archive/destination/open work; running cancellation does not preempt sequential entries, validation, or project open, so inspect entry counts, destination/cleanup state and later workspace/controller effects before repetition. There is no archive-wide atomicity, cleanup, rollback, preemption, retry or fair-lane promise. Approved project-open cancellation is terminal but does not preempt an already-running source read or audio-controller synchronization: inspect its producer-specific result.partial and re-observe the workspace before deciding whether to open again; AIMuse never retries automatically. Approved project-save cancellation is terminal but does not preempt an already-running save: inspect its producer-specific result.partial, destination, project and file audit before deciding whether to save again; AIMuse never retries automatically. Approved plug-in instantiate cancellation is terminal. Waiting cancellation starts no authority continuation or project transaction; running cancellation does not preempt the one atomic descriptor-device transaction, so inspect its producer-specific result.partial and re-observe the destination track/project before repetition. A returned commit remains, a returned lock/conflict commits no device, exceptional failure remains conservative, native hosting is not started, and AIMuse never retries automatically. Plug-in scan cancellation is terminal but does not preempt an already-started helper or atomic catalog write: inspect its producer-specific result.partial again and re-read the globally shared catalog because a complete replacement may settle late, while partial candidate results are never published. Generation cancellation remains terminal if a provider or candidate-cache write fulfills late: inspect its producer-specific result.partial again, because an immutable candidate may be retained but only later explicit acceptance mutates the project, and AIMuse does not retry automatically. Export cancellation is terminal but cooperative once running: inspect result.partial and the destination before repeating because AIMuse neither preempts/cleans output nor retries automatically, and a portable-pack save may already have completed.' },
    ...
    collaboration: { guidance: 'Presence is shared and authenticated. Jobs/results stay owner-only. Human entity/time locks preempt conflicting agent edits; unrelated edits remain allowed. project_apply and actor undo/redo share four fair lanes with bounded actor/global queues and explicit retry guidance; checkpoint/variant create/merge/discard actions share those four fair lanes. Generation start/variation/accept/reject actions share those four fair lanes; plug-in parameter/preset/bypass/remove actions share those four fair lanes. WAV media analysis shares those four fair lanes; audition/consolidation render-job creation share those four fair lanes too. Queued cancellation starts no mutation; for generation start/variation it creates no approval reservation or job and makes no provider request, queued plug-in device cancellation invokes no project transaction, queued WAV analysis performs no source read, managed-cache write, or project transaction, and queued audition/consolidation creates no job/render/cache/project transaction. Pending provider and media-render runtime uses the owner-only job lifecycle without keeping a mutation lane occupied. Running admission work is not preempted. History and checkpoint/variant mutations are not idempotent; generation, plug-in device, and media actions are not generally idempotent. Inspect owned jobs when applicable and re-observe before deciding whether to issue another one. Running WAV analysis can leave content-addressed cache outputs before a failed atomic asset-registration transaction. Running media-job cancellation is cooperative: render output may remain, and already-started asset or consolidation clip transactions may commit; cancelled and failed jobs report retained effects, and a cancelled job never becomes completed or failed later. Media import and separation keep separate authority, multi-file, and unavailable-adapter contracts. Plug-in catalog, scan, and authority-gated instantiate keep their separate discovery/job/approval contracts.' },
...
export interface McpHostOptions { appVersion: string; profileId: string; portSettingsPath: string; cacheRoot: string; projects: ProjectService; audio: AudioEngineController; authority: AuthorityManager; media: MediaManager; plugins: PluginManager; generation: GenerationManager; exports: ExportManager; unpackProjectPack?: (packPath: string, destinationRoot: string, options?: UnpackProjectPackOptions) => Promise<string> }
...
export class McpHost {
  ...
  private buildServer(session: McpSession): McpServer {
    ...
    server.registerTool('media_manage', { title: 'Manage AIMuse media', description: 'List/import/analyze/render project media. WAV analysis uses fair bounded admission; audition/consolidation job creation shares it. Queued render cancellation creates no job or effects; an admitted job does not keep the lane occupied while render remains pending and uses cooperative owner-job cancellation with explicit retained-cache/commit results. Imports accept only authority-approved local paths; approved import cancellation is terminal and reports sequential per-file source/cache/transaction/warning effects without preemption or rollback. Inline bytes are never accepted.', inputSchema: MediaSchema, ...TOOL_OUTPUT, annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true } }, async ({ action, projectId, paths, assetId, trackId, trackIds, startTick, endTick, placeAtTick, adapter }) => {
      ...
      if (action === 'separate') return jsonText({ error: 'adapter_unavailable', adapter, message: 'No source-separation adapter is configured; AIMuse did not substitute a provider.' }, { tool: 'aimuse_help', arguments: { topic: 'tool-contracts' }, guidance: 'Configure and name an explicit adapter in a separately authorized workflow; AIMuse will not substitute a provider.' });
    ...
    server.registerTool('generation_manage', { title: 'Manage generated music and SFX', description: 'Provider-neutral generation lifecycle. Start, variation, accept, and reject use fair bounded admission; queued cancellation creates no approval reservation, job, provider request, candidate disposition, or project mutation. Running cancellation stays terminal across late provider/cache fulfillment and reports producer-specific partial effects for owner re-observation; retained candidates change the project only through later explicit acceptance and no provider request is retried automatically. Job/candidate actions are owner-scoped; provider configuration, credentials, budget and authority remain explicit.', inputSchema: GenerationActionSchema, ...TOOL_OUTPUT, annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true } }, async ({ action, jobId, candidateId, trackId, startTick, request, overrides }) => {
      if (action === 'capabilities') return jsonText(await this.options.generation.capabilities());
      if (action === 'start' && request) return this.scheduleGenerationMutation(session, request.projectId, 'generation start', undefined,
        (actor) => this.startApprovalCapableJob(session, (reservationId) => this.options.generation.start(request as GenerationRequest, actor, reservationId)), (result) => result);
      const owned = jobId ? projects.getJob<GenerationJobResult>(jobId) : undefined;
      if (!owned || owned.ownerActorId !== session.actor.id) return jsonText({ error: 'generation_job_not_found' });
      if (action === 'inspect') return jsonText(jobSummary(owned));
      const generationProjectId = owned.projectId ?? owned.result?.request.projectId;
      if (!generationProjectId) return jsonText({ error: 'generation_job_not_found' });
      if (action === 'accept' && candidateId) return this.scheduleGenerationMutation(session, generationProjectId, 'candidate acceptance', owned.id,
        (actor) => this.options.generation.accept(owned.id, candidateId, trackId, startTick, actor));
      if (action === 'reject' && candidateId) return this.scheduleGenerationMutation(session, generationProjectId, 'candidate rejection', owned.id,
        async () => this.options.generation.reject(owned.id, candidateId));
      if (action === 'variation') {
        const original = owned.result?.request;
        if (!original) return jsonText({ error: 'generation_job_not_found' });
        return this.scheduleGenerationMutation(session, generationProjectId, 'generation variation', owned.id,
          (actor) => this.startApprovalCapableJob(session, (reservationId) => this.options.generation.start({ ...original, ...overrides, seed: overrides?.seed ?? (original.seed === undefined ? undefined : original.seed + 1) }, actor, reservationId)), (result) => result);
      }
      return jsonText({ error: 'invalid_arguments' });
    });
  ...
  private async scheduleGenerationMutation<T>(session: McpSession, projectId: Id, action: string, sourceJobId: Id | undefined, run: (actor: Actor) => Promise<T>, format: (result: T) => ReturnType<typeof jsonText> = (result) => jsonText(result)) {
    const actor = structuredClone(session.actor);
    const scheduled = await this.submitMutation(session, projectId, 1, () => run(actor));
    if (scheduled.accepted) return format(scheduled.result);
    const scheduler = { code: scheduled.code, actorQueueDepth: scheduled.queueDepth, globalQueueDepth: scheduled.globalQueueDepth };
    const startsJob = action === 'generation start' || action === 'generation variation';
    const effect = startsJob
      ? `No ${action}, approval reservation, generation job, or provider request started.`
      : `No ${action}, candidate disposition, or project mutation started.`;
    const next = sourceJobId
      ? { tool: 'generation_manage', arguments: { action: 'inspect', jobId: sourceJobId }, guidance: scheduled.code === 'cancelled'
        ? `${effect} Inspect the owned job and re-observe its project before deciding whether to issue another generation action.`
        : `${effect} Wait at least ${scheduled.retryAfterMs} ms, then inspect the owned job and re-observe its project before deciding whether to issue another generation action; generation actions are not generally idempotent.` }
      : { tool: 'job_manage', arguments: { action: 'list' }, guidance: scheduled.code === 'cancelled'
        ? `${effect} List owned jobs and re-observe the project before deciding whether to issue another generation start.`
        : `${effect} Wait at least ${scheduled.retryAfterMs} ms, then list owned jobs and re-observe the project before deciding whether to issue another generation start; generation starts are not idempotent.` };
    if (scheduled.code === 'cancelled') return jsonText({ status: 'cancelled', message: scheduled.message, scheduler }, next);
    return jsonText({ status: 'busy', message: scheduled.message, conflict: { retryable: true, retryAfterMs: scheduled.retryAfterMs }, scheduler }, next);
  }
