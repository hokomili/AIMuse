...
function helpTopic(topic = 'getting-started', operationKind?: string, projectId = '<observed project ID>', actorId = '<session actor ID>'): Record<string, unknown> {
  const topics: Record<string, Record<string, unknown>> = {
    ...
    rendering: { capabilities: RENDER_CAPABILITIES, loudness: { method: 'ITU-R BS.1770-5 mono/stereo', guidance: 'Master exports normalize with linear gain before master gain/pan/mute. Export job result.loudness reports targetLufs, input, normalized and final encoded output measurements. A 0.98 sample-peak cap can prevent reaching target: peakLimited and warnings disclose the achieved level. This is not a true-peak limiter. Positive master gain can clip; clippedSamples and warnings report it. Below-gate audio is not boosted; sub-400 ms audio is measured with a disclosed zero-padded 400 ms window. SFX manifests carry the report per variant. Raw stems are unnormalized.' } },
