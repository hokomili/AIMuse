...
async function normalizeLufs(path: string, target: number): Promise<void> {
  ...
  for (const channel of decoded.data) for (let index = 0; index < channel.length; index += 1) channel[index] = Math.tanh(channel[index] * gain * 1.02) / Math.tanh(1.02); await atomicWriteFile(path, encodeFloat32Wav(decoded.data, decoded.sampleRate));
}
...
async function applySfxVariation(path: string, deliverable: SfxDeliverable, index: number): Promise<{ pitchSemitones: number; gainDb: number; timingMilliseconds: number; frames: number }> {
  ...
  if (deliverable.seamlessLoop) {
    const start = Math.max(0, Math.min(data[0].length - 1, deliverable.loopStartSample ?? 0));
    const end = Math.max(start + 1, Math.min(data[0].length, deliverable.loopEndSample ?? data[0].length));
    data = data.map((channel) => channel.slice(start, end));
    const crossfade = Math.min(Math.round(decoded.sampleRate * 0.01), Math.floor(data[0].length / 4));
    for (const channel of data) for (let frame = 0; frame < crossfade; frame += 1) { const other = channel.length - crossfade + frame; const amount = 0.5 * (1 - frame / Math.max(1, crossfade)); const beginning = channel[frame]; const ending = channel[other]; channel[frame] = beginning * (1 - amount) + ending * amount; channel[other] = ending * (1 - amount) + beginning * amount; }
  }
  ...
  private async sfx(project: AIMuseProject, request: ExportRequest, jobId: Id): Promise<ExportReport> {
    ...
    for (const deliverable of deliverables) {
      ...
      for (let index = 1; index <= deliverable.variantCount; index += 1) {
        ...
        manifest.push({ deliverableId: deliverable.id, name: deliverable.name, file: `${safeName(filename)}.wav`, variant: index, tags: deliverable.tags, seamlessLoop: deliverable.seamlessLoop, loopStartSample: deliverable.seamlessLoop ? 0 : undefined, loopEndSample: deliverable.seamlessLoop ? variation.frames : undefined, targetLufs: deliverable.targetLufs, variation });
