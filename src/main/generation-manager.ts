...
export class GenerationManager {
  private readonly controllers = new Map<Id, AbortController>(); private readonly actors = new Map<Id, Actor>(); private lyriaEnabled = false;
  ...
  async start(request: GenerationRequest, actor: Actor = HUMAN_ACTOR): Promise<{ jobId: Id }> {
    ...
    const jobId = createId('generation-job'); const timestamp = nowIso(); const job: AsyncJob<GenerationJobResult> = { id: jobId, ownerActorId: actor.id, projectId: request.projectId, kind: 'generation', status: 'queued', progress: 0, message: 'Generation queued.', createdAt: timestamp, updatedAt: timestamp, cancellable: true, result: { request: structuredClone(request), candidates: [], acceptedCandidateIds: [], rejectedCandidateIds: [] } };
  ...
  async run(jobId: Id, actor: Actor, authorityOverride: boolean): Promise<void> {
    const job = this.projects.getJob<GenerationJobResult>(jobId); if (!job?.result || !['queued', 'waiting-for-user'].includes(job.status)) return; const request = job.result.request; const credential = await this.credentials.get(request.provider); if (!credential) return this.fail(job, new ProviderError('Provider credential is unavailable.', 'credential', false));
    const controller = new AbortController(); this.controllers.set(jobId, controller); this.projects.upsertJob({ ...job, status: 'running', progress: 0.02, message: `Generating with ${request.provider}/${request.model}…`, updatedAt: nowIso(), approval: undefined });
    ...
    try {
      ...
      for (let index = candidates.length; index < request.resultCount; index += 1) {
        if (controller.signal.aborted) throw new DOMException('Cancelled', 'AbortError'); if (actor.kind === 'agent' && !authorityOverride) { const decision = this.authority.generation(request.provider, request.model, request.estimatedCostMinor); if (!decision.allowed) throw new ProviderError(decision.reason ?? 'Authority exhausted.', 'authority-exhausted', false); }
        const raw = await this.generateOne(request, credential, controller.signal); if (actor.kind === 'agent' && !authorityOverride) this.authority.consumeGeneration(request.estimatedCostMinor);
        candidates.push(await this.persistCandidate(request, raw, actor)); const current = this.projects.getJob<GenerationJobResult>(jobId)!; this.projects.upsertJob({ ...current, progress: 0.05 + 0.9 * candidates.length / request.resultCount, message: `Generated candidate ${candidates.length} of ${request.resultCount}.`, updatedAt: nowIso(), result: { ...current.result!, candidates } });
      }
      const current = this.projects.getJob<GenerationJobResult>(jobId)!; this.projects.upsertJob({ ...current, status: 'completed', progress: 1, message: `${candidates.length} candidate${candidates.length === 1 ? '' : 's'} ready for review.`, updatedAt: nowIso(), result: { ...current.result!, candidates } });
    } catch (error) { if (controller.signal.aborted || (error instanceof DOMException && error.name === 'AbortError')) { const current = this.projects.getJob<GenerationJobResult>(jobId)!; this.projects.upsertJob({ ...current, status: 'cancelled', message: 'Generation cancelled. A submitted provider request was not retried.', updatedAt: nowIso(), result: { ...current.result!, candidates } }); } else this.fail(this.projects.getJob<GenerationJobResult>(jobId)!, error, candidates, credential); }
    finally { this.controllers.delete(jobId); }
  }

  cancel(jobId: Id): AsyncJob | undefined { this.controllers.get(jobId)?.abort(); return this.projects.cancelJob(jobId); }
  ...
  private async generateOne(request: GenerationRequest, credential: string, signal: AbortSignal): Promise<RawGeneration> { if (request.provider === 'elevenlabs') return this.elevenLabs(request, credential, signal); if (request.provider === 'stability') return this.stability(request, credential, signal); return this.lyria(request, credential, signal); }

  private async elevenLabs(request: GenerationRequest, credential: string, signal: AbortSignal): Promise<RawGeneration> {
  ...
  private async stability(request: GenerationRequest, credential: string, signal: AbortSignal): Promise<RawGeneration> {
    ...
    let response: Response; try { response = await this.fetcher(endpoint, { method: 'POST', headers: { authorization: `Bearer ${credential}`, accept: 'audio/*', 'stability-client-id': 'AIMuse' }, body: form, signal }); } catch (error) { throw new ProviderError(`Stability request failed: ${error instanceof Error ? error.message : String(error)}`, 'network', true, true); }
